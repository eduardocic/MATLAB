...
// entrada e saídas, é necessário que se especifique o tamanho das mesmas
...
//            Ou seja, 'plhs' é um ponteiro de tamanho 'nlhs'. Isto é, 
//            guarda 'nlhs' endereços diferentes saídas.
//     
...
//            seja, 'prhs' é um ponteiro de tamanho 'nrls'. Isto é, guarda 
//            'nrls' endereços diferentes de entradas.    
// ------------------------------------------------------------------------ 
  double *y, *x, *z;
  size_t mrows, ncols;
  ...
  // Perceba aqui que a PRIMEIRA variável em C (ou C++) começa sempre 
  // com zero. Além do mais, foi verificado o tamanho da variável (faço 
  // isso por meio da verificação das linhas e colunas da variável 
  // chamada, conforme linhas abaixo).
  mrows = mxGetM(prhs[0]);       // pega o número de linhas de 'prhs[0]'
  ncols = mxGetN(prhs[0]);       // pega o número de colunas de 'prhs[0]'
