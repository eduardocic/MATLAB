...
n    = max(size(x));
m    = max(size(u)) - 1;         % Retirei aqui o centro de massa Xcg.
mm   = m;
tol  = 1e-6;
time = 0.;

% mm   = input('Number of control to be used? ');        % Para o cálculo da Matriz B.
...
%    C = ----------------
%             d.x
last  = zeros(saidas, 1);    % Desejo ter 'saidas' saídas.
for j = 1:n
    ut = u;
    for i = 1:rep
        xt(j)    = x(j) + dx(j);
...
%    D = ----------------
%             d.u
last  = zeros(saidas, 1);               % Desejo ter 2 saídas.
for j = 1:max(size(last))                        % Desejo ter 2 saídas.
    ut = u;
    for i=1:rep
        ut(j)    = u(j) + du(j);
...
save('MatrizesDeEstado.mat', 'A', 'B', 'C', 'D');
