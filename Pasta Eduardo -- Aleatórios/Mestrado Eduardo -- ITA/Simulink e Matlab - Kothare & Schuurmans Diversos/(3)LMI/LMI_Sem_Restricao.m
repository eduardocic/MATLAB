function [sys, past_x0, str, ts] = LMI_Sem_Restricao(t, past_x, inputs, flag, A, B, T)
...
[sys,past_x0,str,ts] = mdlInitializeSizes(T,p,n); % Inicialização.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
case {1,4,9}
sys = [];           % Flags que não são utilizadas.
...
SistemaLMIs = getlmis;      % Sintetiza as LMIs para solução do problema.


% Cálculo do custo a ser minimizado.
NumVar = decnbr(SistemaLMIs);
c = zeros(NumVar,1);
c(1) = 1;                   % Custo da forma J = 1*gamma.

% Opções de cálculo a ser feito.
options = [0 200 0 0 1];           
[Copt, Xopt] = mincx(SistemaLMIs, c, options);

% Pega valores solução.
