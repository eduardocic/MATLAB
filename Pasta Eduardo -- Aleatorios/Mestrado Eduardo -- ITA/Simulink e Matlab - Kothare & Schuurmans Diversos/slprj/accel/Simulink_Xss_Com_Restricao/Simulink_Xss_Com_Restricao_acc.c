...
_rtDW -> DiscreteStateSpace_DSTATE [ 0 ] + ( _rtP -> P_0 [ 1 ] ) * _rtDW ->
DiscreteStateSpace_DSTATE [ 1 ] ; xnew [ 1 ] = ( _rtP -> P_0 [ 2 ] ) * _rtDW
-> DiscreteStateSpace_DSTATE [ 1 ] ; xnew [ 1 ] += _rtP -> P_1 * _rtB ->
B_0_1_0 [ 0 ] ; ( void ) memcpy ( & _rtDW -> DiscreteStateSpace_DSTATE [ 0 ]
, xnew , sizeof ( real_T ) * 2 ) ; } ssCallAccelRunBlock ( S , 0 , 1 ,
SS_CALL_MDL_UPDATE ) ; } UNUSED_PARAMETER ( tid ) ; } static void
mdlInitializeSizes ( SimStruct * S ) { ssSetChecksumVal ( S , 0 , 1437916007U
) ; ssSetChecksumVal ( S , 1 , 866166167U ) ; ssSetChecksumVal ( S , 2 ,
920855791U ) ; ssSetChecksumVal ( S , 3 , 3290975364U ) ; { mxArray *
