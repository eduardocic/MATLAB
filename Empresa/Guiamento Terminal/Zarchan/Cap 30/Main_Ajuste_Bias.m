...
% Cálculo do vetor LOS entre o míssil e o alvo.
dx = Y(1,1) - X(1,1);
dy = Y(1,3) - X(1,3);
d  = sqrt(dx^2 + dy^2);
...
% Cálculo de aproximação entre os alvos e velocidade do míssil.
Vc  = -(dx*vx + dy*vy)/d;
Vm  = sqrt(X(1,2)^2 + X(1,4)^2);
...
biasI = (N*lambda - gammaIC - (N-1)*gammaF)/(d/Vc);
Vi    = Vc;
...
while(Vc >= 0)
    ...
    Vc(i)   = -(dx*vx + dy*vy)/d;
    Vm(i)   = sqrt(X(i,2)^2 + X(i,4)^2);
    
    % Cálculo do 'nc' e beta.
    % -----------------------
    bias(i)    = biasI*Vm(i)/Vi;            % Bias inicial -- Vmíssil - Vmissil, inicial.
    N          = Nlinha*Vc(i)/Vm(i);
    gammaD(i)  = N*lambdaD + bias(i);
    nc(i)      = Vm(i)*gammaD(i);
