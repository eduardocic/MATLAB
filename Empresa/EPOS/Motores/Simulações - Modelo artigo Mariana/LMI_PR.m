function [Qsol, Ysol] = LMI_PR(A, B, C, S, R, Umax, Eps, yb)
...
n = size(A{1},1);         % Dimensão do sistema. 
p = size(B{1},2);         % Quantidade de entradas u(k)'s.
N = max(size(A));         % Vértices total do politopo.   
...
% d) LMI (iv);                                    +
%                                                 +
%  Essa LMI faz referência ao lim. de entrada.    +
%                                                 +
%     | 1   Umax,l  | <= 0              (iv)      + 
%     | *    X,ll   |                             +
%     para todo l = 1,...,p.                      +
...
options = [0 10000 -1 0 1];           
[Copt, Xopt] = mincx(SistemaLMIs, c, options, Xsol);
Xsol = Xopt;
